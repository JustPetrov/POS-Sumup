...
async function sumup(path, options = {}) {
  ...
  if (!response.ok) {
    throw new Error(data?.message || data?.error || `SumUp HTTP ${response.status}`);
  }
  return data;
}

function cors(res) {
  const allowed = process.env.POS_ORIGIN;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const affiliate = affiliateAppId && affiliateKey
      ? { app_id: affiliateAppId, key: affiliateKey, foreign_transaction_id: foreignId }
      : undefined;
    ...
    const payload = {
      total_amount: {
        currency: 'EUR',
        minor_unit: 2,
        value: Math.round(numericAmount * 100),
      },
    ...
    if (!checkout?.client_transaction_id) {
      throw new Error('SumUp returned no client_transaction_id');
    }

    return res.status(200).json({
      success: true,
      readerId,
      clientTransactionId: checkout.client_transaction_id,
      checkout,
    });
  } catch (error) {
